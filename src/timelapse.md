...
const titre = html`<h1>Time Lapse</h1>`;
display(titre);
```

```js
// Available years for the time-lapse
const available_years = ['2018','2021', '2024'];

const timeSlider = view(Inputs.range({
  min: 0,
  max: available_years.length - 1,
  step: 1,
  value: 0,
  label: "Year",
  format: value => available_years[value],
}));

// Display the selected year
const selectedYear = available_years[timeSlider];
...
display(playPauseButton);

// Time-lapse function
function startTimeLapse() {
  timeLapseInterval = setInterval(() => {
    let currentYearIndex = timeSlider.value;
    currentYearIndex = (currentYearIndex + 1) % available_years.length;
    timeSlider.value = currentYearIndex;
    updateMapLayer(currentYearIndex);
  }, 1000); // 1000 ms per year in the time-lapse
}
```
